...
const BookApppointment = ({date}) => {
    ...
    return (
        <div>
            <h2 className="text-color text-center mt-5">Available Appointment on {date.toDateString()}</h2>

            <div className="row mt-5  justify-content-center">
                {
                    bookingData.map(data=><BookingCard key={data._id} date={date} book={data}></BookingCard>)
                }
